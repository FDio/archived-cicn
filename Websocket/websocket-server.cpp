...
void
WebSocketServer::setHandler(const HandlerFunction &handler)
{
...
void
WebSocketServer::onMessage(Server *s, websocketpp::connection_hdl hdl, message_ptr msg)
{
  try {
    handler(s, hdl, msg, (uint8_t *) msg->get_payload().c_str(), msg->get_payload().size());
  } catch (const websocketpp::lib::error_code &e) {

    std::cout << "Reply sent failed because: " << e
              << "(" << e.message() << ")" << std::endl;
